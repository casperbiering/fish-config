...
abbr -a -g gphf 'git push --force-with-lease'
abbr -a -g gphff 'git push --force'
...
abbr -a -g tfi 'tt terraform init'
...
abbr -a -g tfc 'jq -r \'.Modules[]|select(.Version==null)|select(.Source|test("ref=v[0-9]+[.][0-9]+[.][0-9]+")|not)|select(.Source|test("^(\\\\\\\.?\\\\\\\./|$)")|not)|.Dir|capture("^(?<a>.terraform/modules/[^/]+)(/|$)").a\' .terraform/modules/modules.json | xargs -tn 1 rm -r ; terraform get'
